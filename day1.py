...
def text_int_to_int(line, line_offset):
    text_ints = ["zero", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine"]
    for idx, text_int in enumerate(text_ints):
        # does this subsection of the line match a text int?
        if line[line_offset:line_offset+len(text_int)] == text_int:
            return idx
    return -1
...
for line in input.split('\n'):
    ...
    for idx, letter in enumerate(line):
        text_int = text_int_to_int(line, idx)
        if text_int != -1:
            # we parsed a text int, so let's just set letter to that
            letter = str(text_int)
        if letter.isdigit():
            second = int(letter)
            if first == -1:
