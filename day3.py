...
def check_for_symbols(line, start_offset, end_offset) :
...
def is_part_number(lines, line_index, start_offset, end_offset):
    # check the line above for symbols 
    if line_index > 0:
        prev_line = lines[line_index-1]    
        if check_for_symbols(prev_line, max(start_offset-1, 0), min(end_offset+1, len(prev_line)-1)):
            return True # this is a part number

    # check the line below for symbols 
    if line_index+1 < len(lines):
        next_line = lines[line_index+1]
        if check_for_symbols(next_line, max(start_offset-1, 0), min(end_offset+1, len(next_line)-1)):
            return True # this is a part number
 
    # check this line for symbols 
    if start_offset > 0:
        if lines[line_index][start_offset-1] != '.' and not lines[line_index][start_offset-1].isdigit():
            return True # this is a part number
    if end_offset+1 < len(lines[line_index]):
        if lines[line_index][end_offset+1] != '.' and not lines[line_index][end_offset+1].isdigit():
            return True # this is a part number

def parse_number(line, letter_index) :
    if not line[letter_index].isdigit():
        return -1
    ...
    if not line[letter_index].isdigit():
        letter_index+=1
    # ok we're at the start of the number
    number = 0
    while letter_index < len(line):
        if not line[letter_index].isdigit():
            break
        number*=10
        number+=int(line[letter_index])
        letter_index+=1
    return number

def find_parts_around(lines, line_index, letter_index):
    parts = []
    # check the line above
    if line_index > 0:
        prev_line = line_index-1
        num = parse_number(lines[prev_line], max(letter_index-1, 0))
        num2 = parse_number(lines[prev_line], min(letter_index+1, len(lines[prev_line])))
        if num != -1:
            parts.append(num)
        if num != num2 and num2 != -1:
            # there are two numbers above
            parts.append(num2)
    # check this line
    if letter_index > 0:
        num = parse_number(lines[line_index], letter_index-1)
        if num != -1:
            parts.append(num)
    if letter_index+1<len(lines[line_index]):
        num = parse_number(lines[line_index], letter_index+1)
        if num != -1:
            parts.append(num)
    ...
    if line_index > 0:
        ...
        num = parse_number(lines[next_line], max(letter_index-1, 0))
        num2 = parse_number(lines[next_line], min(letter_index+1, len(lines[next_line])))
        if num != -1:
            parts.append(num)
        if num != num2 and num2 != -1:
            # there are two numbers above
...
for line_index, line in enumerate(lines):
    for letter_index, letter in enumerate(line):
        if letter == '*':
            parts = find_parts_around(lines, line_index, letter_index)
            if len(parts) == 2:
