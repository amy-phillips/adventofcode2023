input="""ztj{s<2703:A,m<1025:vk,ztl}
...
class Instruction:
    threshold:int
    less_than:bool
    variable:xmas = xmas.ALL
    destination:str

    def __init__(self, _threshold: int, _less_than: bool, _variable:xmas, _destination:str):
        self.threshold = _threshold
        self.less_than = _less_than
...
class Workflow:
    ...
    def __init__(self, _name: str, _instructions: list[Instruction]):
        self.name = _name
...
def parse_workflows(lines):
    ...
    for line_i,line in enumerate(lines):
        if len(line)==0:
            break # finished workflows

        name,instructions_string=line.split('{')
        ...
        for instruction_str in instructions_string:
            if ':' in instruction_str:
                instruction_str,destination=instruction_str.split(':')
                variable:xmas
                if instruction_str[0]=='x':
                    variable=xmas.X
                elif instruction_str[0]=='m':
                    variable=xmas.M
                elif instruction_str[0]=='a':
                    variable=xmas.A
                elif instruction_str[0]=='s':
                    variable=xmas.S
                less_than = instruction_str[1]=='<'
                threshold=int(instruction_str[2:])
            else:
                # must be the final catch all instruction
                destination=instruction_str
                variable=xmas.ALL
                less_than=False
                threshold=0
            instructions.append(Instruction(threshold, less_than, variable, destination))

        workflows.append(Workflow(name, instructions))     
